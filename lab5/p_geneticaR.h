...
void pgenetica::mostrarPoblacion(){
	...
	{	
		...
		file<<"c:"<<pob[i].cadena<<" f:"<<pob[i].fitness<<" p: "<<pob[i].porcent<<" inv: "<<pob[i].inv<<endl;
	}
}
...
void pgenetica::generarPorcentajes()
{
	...
	for (int i = 0; i < pob.size(); ++i)
		sum_inv += pob[i].inv;
	file<<"suma: "<<sum_inv<<endl;
	for (int i = 0; i < pob.size(); ++i){
		// file<<pob[i].fitness<<" ";
		pob[i].porcent = (pob[i].inv*100)/sum_inv;
...
{
	// int ale = rand()%101;
	int ale = ((double) rand()*(101)/(double)RAND_MAX-0);
	float limite = 0;
...
void pgenetica::cruzamiento()
{
    ...
    do{
        ...
        p1 = metodoRuleta();
        do{p2 = metodoRuleta();}while (p1 == p2);
        h1 = pob[p1]; h2 = pob[p2];
